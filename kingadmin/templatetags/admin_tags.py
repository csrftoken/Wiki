...
def get_table_column(column, table_obj):
    if hasattr(table_obj.model_class, column):
        ...
        # check if this field exist in management class
        ...
        for index, column_name in enumerate(table_obj.list_display):
            ...
            if hasattr(row_obj, column_name):
                ...
                if onclick_column == column_name:
                    ...
                    column = '''<td><a class='btn-link'  href='%schange/%s/' >%s</a> </td> ''' % (
...
def pagenator(obj, data_type):
    ...
    if obj.has_previous():
        html += '''<li class="disabled"><a href="?page=%s&type=%s" class="fa fa-angle-double-left"></a></li>''' % (
        obj.previous_page_number(), data_type)
    ...
    if obj.has_next():
        html += '''<li class="disabled"><a href="?page=%s&type=%s" class="fa fa-angle-double-right"></a></li>''' % (
        obj.next_page_number(), data_type)
...
def pagenator2(obj, arg_name, arg_val):
    html = '''<ul class="pagination">'''
    if obj.has_previous():
        html += '''<li class="disabled"><a href="?page=%s&%s=%s" class="fa fa-angle-double-left"></a></li>''' % (
        obj.previous_page_number(), arg_name, arg_val)
    ...
    if obj.has_next():
        html += '''<li class="disabled"><a href="?page=%s&%s=%s" class="fa fa-angle-double-right"></a></li>''' % (
        obj.next_page_number(), arg_name, arg_val)
...
#     field_type = field_obj.get_internal_type()
#     if field_type in ("ForeignKey","ManyToManyField"):
#         if field.name not in form_obj.Meta.management.readonly_fields:
...
def add_onclick_link(form_obj, field_obj):
    ...
    if field_obj.name in form_obj.Meta.admin.change_page_onclick_fields:
        # link_url = url_reverse(form_obj.Meta.management.change_page_onclick_fields[field_obj.name][0],
...
@register.simple_tag
def get_model_display_name(table_obj):
    return table_obj.admin_class.model_display_name or table_obj.model_class._meta.verbose_name
